  ...
  methods: {
    ...
    getTreeTableData(data, parentKey, res) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        res.push(resItem)
        if (Object.hasOwnProperty.call(row, childrenKey) && row[childrenKey] && row[childrenKey].length > 0) {
      ...
      if (this.treeExpand) {
        if (this.treeExpand.expandKey === column.property) {
          ...
          this.tableData.forEach((element, i) => {
            const isChildren = element.uniqueId.indexOf(row.uniqueId + '-') === 0
            if (row.expanded) {
    ...
    toggleRowSelection(row, selected) {
      let toggleRow = row
      if (this.rowKey) {
        const filterRow = this.tableData.filter((e) => e[this.rowKey] === row[this.rowKey])
    ...
    toggleRowAndChildrenSelection(row, selected) {
      ...
      const rowChildren = this.tableData.filter((e) => e.uniqueId.indexOf(rowUniqueId + '-') === 0)
    ...
    getChildRows(row) {
      return this.tableData.filter((e) => e.uniqueId.indexOf(row.uniqueId + '-') === 0)
    },
    // 下面是自定义级联复选框的
    handleCheckedAll(val) {
      console.log('handleCheckedAll', val)
      ...
      this.$emit('handle-selection-change', val ? this.tableData : [])
    },
    handleCheckedRow(val, row) {
      console.log('handleCheckedRow', row)
      ...
      this.indeterminateAll = checkedData.length > 0 && checkedData.length !== this.tableData.length
    ...
    setParentChecked(parentUniqueId) {
      if (parentUniqueId) {
        const parentFilters = this.tableData.filter((e) => e.uniqueId === parentUniqueId)
        if (parentFilters.length > 0) {
          const parent = parentFilters[0]
          //  所有子集
          const parentAllChild = this.tableData.filter((e) => e.parentUniqueId === parentUniqueId)
          // 包括半选和全选
          const parentAllChildChecked = this.tableData.filter((e) => e.parentUniqueId === parentUniqueId && e.checked)
          // 只是半选
          const parentAllChildIndeterminate = parentAllChildChecked.filter((e) => e.isIndeterminate)
          ...
          if (CheckedLength > 0) {
            ...
            if (IndeterminateLength > 0 && IndeterminateLength < CheckedLength) {
              parent.isIndeterminate = true
            }
          } else {
            parent.isIndeterminate = false
