...
const openProcess = process.env.NODE_ENV == 'dev' || process.env.NODE_ENV == 'vmsit' || process.env.NODE_ENV == 'sit'
// 按钮权限
...
Vue.directive('areaSelect', {
  inserted: (el, binding, vnode) => {
    ...
    el.addEventListener('mouseup', function(evt) {
      ...
      mouseDownFlag = false
      // 框选逻辑
      let posList = Array.from(randIds)
        .filter((v) => v[0])
        .map((v) => v[0])
        .map((v) => v.split('_'))
      let posYList = posList.map((v) => v[0])
      let posXList = posList.map((v) => v[1])
      let minX = Math.min(...posXList),
        minY = Math.min(...posYList)
      let maxX = Math.max(...posXList),
        maxY = Math.max(...posYList)
