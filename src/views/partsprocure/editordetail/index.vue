	...
	<iPage>
					...
					<div class="col">
						...
						<iFormItem label="采购条款：" name="test">
							<iSelect v-model="detailData.purchaseClause">
								<el-option :value="item.value" :label="item.label"
									v-for="(item, index) in getGroupList('purchase_clause')" :key="index"></el-option>
							</iSelect>
						</iFormItem>
					</div>
				</div>
		...
		<iTabsList class="margin-top20" type="border-card">
			...
			<el-tab-pane label="申请目标价">
				<targetPrice :purchaseProjectId="infoItem.purchasePrjectId"></targetPrice>
			</el-tab-pane>
		...
		<iDialog class="dialog" :visible.sync="splitPurchBoolean">
			...
			<template>
				<el-table :height="height" :data="splitPurchList" v-loading="tableLoading"
					@selection-change="handleSelectionChange" width="500" class="margin-right30">
