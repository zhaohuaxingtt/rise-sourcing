...
<template>
	<iPage>
		...
		<<<<<<< HEAD <!------------------------------------------------------------------------>
			...
			<splitFactory :splitPurchBoolean="splitPurchBoolean" :purchaseProjectId="purchaseProjectId"
				:update='updateTabs' :close='splitPurch'></splitFactory>
	</iPage>
</template>
<script>
	import {
	...
	name = "test" >
		...
		el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('part_preject_type')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		iFormItem label = "采购工厂："
	name = "test" >
		<
		iSelect v - model = "detailData.procureFactory" >
		<
		el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('procure_factory')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		iFormItem label = "Common Sourcing："
	name = "test" >
		<
		iSelect v - model = "detailData.commonSourcing" >
		<
		el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('is_common_sourcing')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		iFormItem label = "支付条款："
	name = "test" >
		<
		iSelect v - model = "detailData.payClause" >
		< el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('pay_clause')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		/div> <
		div class = "col" >
		<
		iFormItem label = "FSNR/GSNR/SPNR："
	name = "test" >
		<
		iText > {
			{
	...
	name = "test" >
		...
		el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('part_type')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		iFormItem label = "单位："
	name = "test" >
		...
		el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('unit')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		iFormItem label = "MTZ零件："
	name = "test" >
		<
		iSelect v - model = "detailData.mtz" >
		<
		el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('mtz')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		/div> <
	...
	name = "test" >
		...
		el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('linie_dept')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
	...
	name = "test" >
		...
		el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('linie_name')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		iFormItem label = "CF控制员："
	name = "test" >
		<
		iSelect v - model = "detailData.cfController" >
		< el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('cf_controller')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		iFormItem label = "货币："
	name = "test" >
		...
		< el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('currency_id')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		/div> <
	...
	name = "test" >
		...
		/iText> <
		/iFormItem> <
		iFormItem label = "采购条款："
	name = "test" >
		<
		iSelect v - model = "detailData.purchaseClause" >
		< el - option: value = "item.value": label = "item.label"
	v -
		for = "(item, index) in getGroupList('purchase_clause')": key = "index" >
		< /el-option> <
		/iSelect> <
		/iFormItem> <
		/div> <
	...
	type = "border-card" >
		...
		outputPlan ref = "outputPlan": params = "infoItem" / >
		<
		outputRecord ref = "outputRecord"
	class = "margin-top20": params = "infoItem"
	...
	title = "结束项目" >
		< /backItems> <
		splitFactory: splitPurchBoolean = "splitPurchBoolean": purchaseProjectId = "purchaseProjectId": update =
		'updateTabs': close = 'splitPurch' >
		< /splitFactory> <
		/iPage> <
		/template> <
		script >
	...
	>>> > 0503 c05b41c0bf3030865589a6bdcbc58b72765b
</script>
