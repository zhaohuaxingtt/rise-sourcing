	...
	<iPage class="partsprocureEditordetail" v-permission.auto="PARTSPROCURE_EDITORDETAIL_INDEXPAGE|零件采购项目管理详情页">
		...
		<iCard class="card" :title="language('LK_JICHUXINXI','基础信息')" collapse v-loading='detailLoading'>
			<iFormGroup row="1" inline :rules="rules">
				<div class="row">
					<div class="col">
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_PARTNUMBER|零件号" :label="language('LK_LINGJIANHAO','零件号') + ':'" name="test">
							<iText>
								{{ detailData.partNum }}
							</iText>
						</iFormItem>
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_PARTNAMEZH|零件名中" :label="language('LK_LINGJIANMINGZHONG','零件名（中）') + ':'" name="test">
							<iText >
								{{ detailData.partNameZh }}
							</iText>
						</iFormItem>
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_PARTNUMBERGER|零件名称德" :label="language('LK_LINGJIANMINGCHENGDE','零件名称（德）') + ':'" name="test">
							<iText >
								{{ detailData.partNameDe }}
							</iText>
						</iFormItem>
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_PARTTYPE|零件类型" :label="language('LK_LINGJIANLEIXING','零件类型') + ':'" name="test">
						...
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_UNIT|价格单位" :label="language('LK_DANWEI','价格单位') + ':'" name="test">
						...
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_BMG|BMG" label="BMG：" name="test">
							<iText >
								{{ fillterss(detailData.isBmg) }}
							</iText>
						</iFormItem>
						<iFormItem  v-permission.auto="PARTSPROCURE_EDITORDETAIL_CURRENCYCODE|货币" v-if="[partProjTypes.DBLINGJIAN].includes(detailData.partProjectType)" :label="language('LK_HUOBI','货币') + ':'" name="test">
						...
						<iFormItem  v-permission.auto="PARTSPROCURE_EDITORDETAIL_ISDB|是否DB件"  v-if="[partProjTypes.YICIXINGCAIGOU, partProjTypes.DBYICHIXINGCAIGOU].includes(detailData.partProjectType)" :label="language('SHIFOUDBJIAN','是否DB件') + ':'" name="test">
						...
						<!-----------------------采购项目为仅零件号变更-------------------------------------->
						<iFormItem   v-permission.auto="PARTSPROCURE_EDITORDETAIL_OLDFSGSNUMBER|原FS/GS号"  v-if='partProjTypes.JINLINGJIANHAOGENGGAI == detailData.partProjectType' :label="language('YUANFSGSHAO', '原FS/GS号') + ':'">
                ...
                :disabled="isGXWW"
						...
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_PARTSTATUS|零件项目类型描述" :label="language('partsprocure.PARTSPROCUREPARTSTATUSNAME','零件项目类型') + ':'" name="test">
							<iText>
					...
					<div class="col">
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_INQUIRYBUYER|询价采购员" :label="language('LK_XUNJIACAIGOUYUAN','询价采购员') + ':'" name="test">
							<iText >
						...
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_LINE|LINIE" label="LINIE：" name="test">
							...
							<iSelect ref="linieSelect" v-model="detailData.linieId" filterable placeholder='请先选择LINIE部门' v-if="!disabled && (detailData.status != getEnumValue('PURCHASE_PROJECT_STATE_ENUM.HAS_RFQ'))" @change="handleChangeByLinie">
					...
					<div class="col">
						...
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_LINGJIANCHENGBENFENXIYUAN|零件成本分析员" :label="language('LINGJIANCHENGBENFENXIYUAN', '零件成本分析员') + ':'" name=''>
						...
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_MTZPARTS|MTZ零件" :label="language('LK_MTZLINGJIAN','MTZ零件') + ':'" name="test">
						...
						<iFormItem v-permission.auto="PARTSPROCURE_EDITORDETAIL_DATEOFRECEIPT|签收日期" :label="language('LK_QIANSHOURIQI','签收日期') + ':'" name="test">
							<iText>
								{{ detailData.signDate }}
							</iText>
						</iFormItem>
						...
						<!---------------钢材批量采购及钢材一次性采购显示字段---------------------------------------------->
						<template v-permission.auto="PARTSPROCURE_EDITORDETAIL_ULTIMATELYUSER|最终用户" v-if="[partProjTypes.GANGCAIYICIXINGCAIGOU, partProjTypes.GANGCAIPILIANGCAIGOU].includes(detailData.partProjectType)">
							<iFormItem  :label="language('ZUIZHONGYONGHU','最终用户') + ':'" name="test">
								<iText >
									{{ detailData.endUser }}
								</iText>
							</iFormItem>
							<iFormItem  v-permission.auto="PARTSPROCURE_EDITORDETAIL_MATERIALSUPPLIER|材料供应商" :label="language('CAILIAOGONGYINGSHANG','材料供应商') + ':'" name="test">
								<iText>
									{{ detailData.materialSupplierName }}
								</iText>
							</iFormItem>
							<iFormItem  v-permission.auto="PARTSPROCURE_EDITORDETAIL_STARTANDENDDATE|有效起始期" :label="language('YOUXIAOQISHIQI','有效起始期') + ':'" name="test">
								<iText>
		...
		watch:{
			...
			partProjectTypePack(data) {
				if (this.$refs.partProjectTypeSelect && this.$refs.partProjectTypeSelect.$el && this.$refs.partProjectTypeSelect.$el.querySelector("input")) {
					const inputDom = this.$refs.partProjectTypeSelect.$el.querySelector("input")

					if (data.partProjectTypeCode) {
						inputDom.value = this.detailData.partProjectTypeDesc || ""
						const current = data.partProjectTypeOptions.find(option => option.code === data.partProjectTypeCode)
						if (current) inputDom.value = current.name
					} else {
						inputDom.value = ""
					}
			...
			liniePack(data) {
				if (this.$refs.linieSelect && this.$refs.linieSelect.$el && this.$refs.linieSelect.$el.querySelector("input")) {
			...
			cfControllerPack(data) {
				if (this.$refs.cfControllerSelect && this.$refs.cfControllerSelect.$el && this.$refs.cfControllerSelect.$el.querySelector("input")) {
					...
					if (data.cfControllerId) {
						...
						const current = data.cfControllerOptions.find(option => option.id === data.cfControllerId)
						if (current) inputDom.value = current.name
					} else {
						inputDom.value = ""
					}
