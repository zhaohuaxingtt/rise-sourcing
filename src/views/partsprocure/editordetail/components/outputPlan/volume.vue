<template>
  <iCard class="volume" tabCard>
    <template #header>
      <div class="title">
        <p>
          {{ language("LK_LINGJIANMEICHEYONGLIANG", "零件每车用量") }}
          <template v-if="params.partProjectSource == 1">{{
            `（${language(
              "LK_DANGQIANBANBEN",
              "当前版本"
            )} : ${versionComputed}）`
        ...
        <div class="control">
          <div v-if="!disabled" class="btn-left">
            <iButton
              ...
              @click="calculation()"
              v-permission.auto="
                PARTSPROCURE_OUTPUTPLAN_VOLUME_CALCULATION | 计算产量
              "
              >{{ language("LK_JISUANCHANLIANG", "计算产量") }}</iButton
            >
            <iButton
              v-if="isEdit"
              @click="addCar"
              v-permission.auto="PARTSPROCURE_OUTPUTPLAN_VOLUME_ADD | 添加"
            ...
            <iButton
              ...
              @click="saveData"
              :loading="saveLoading"
              v-permission.auto="PARTSPROCURE_OUTPUTPLAN_VOLUME_SAVE | 保存"
              >{{ language("LK_BAOCUN", "保存") }}</iButton
            >
            <iButton
              v-if="isEdit"
              @click="deleteData()"
              v-permission.auto="PARTSPROCURE_OUTPUTPLAN_VOLUME_DELETE | 删除"
              >{{ language("LK_SHANCHU", "删除") }}</iButton
            >
            <iButton
              v-if="isEdit"
              @click="fillDown()"
              v-permission.auto="
                PARTSPROCURE_OUTPUTPLAN_VOLUME_FILLDOWN | 向下填充
              "
              >{{ language("LK_XIANGXIATIANCHONG", "向下填充") }}</iButton
            >
            <iButton
              v-if="!isEdit && ispartProjectSource"
              @click="edit()"
              v-permission.auto="
                PARTSPROCURE_OUTPUTPLAN_VOLUME_EDIT | (编辑 / 取消)
              "
      ...
      >
        <template #perCarDosage="scope">
          <iInput
            ...
            @click.native.stop
            @focus="handleFocusByInput(scope.row)"
            @input="handleInputByPerCarDosage($event, scope.row)"
          />
...
export default {
  ...
  computed: {
    canComputed() {
      // 已定点，已冻结不显示
      return !["NOMINATED", "FROZEN_NOM"].includes(this.params.status);
    },
    // ispartProjectSource() {
    //    if(this.params.partProjectSource == 2) {
    //   return true
  ...
  methods: {
    async getData() {
      ...
      if (this.params.partProjectSource == 1) {
        try {
          if (
            (!this.version || !this.carTypeConfigId) &&
            this.params.purchasingRequirementId
          ) {
          ...
          if (infoRes.code != 200) {
            return iMessage.error(
              `${this.$i18n.locale === "zh" ? infoRes.desZh : infoRes.desEn}`
            );
          }

          if (infoRes.data) {
            infoRes.data.tpRecordList.forEach((val) => {});
            this.tableListData = Array.isArray(infoRes.data.tpRecordList)
              ? infoRes.data.tpRecordList
              : [];
        ...
        manualInfoTable({
          ...
          .then((res) => {
            if (res.code == "200") {
              ...
              this.tableListData = Array.isArray(res.data.tpRecordList)
                ? res.data.tpRecordList
                : [];
              this.tableListDataCache = cloneDeep(
                Array.isArray(res.data.tpRecordList)
                  ? res.data.tpRecordList
                  : []
              );
    ...
    calculation() {
      ...
      if (flag == false) {
        iMessage.error(
          this.language("QINGXIANBAOCUNMEICHEYONGLIANG", "请先保存每车用量")
        );
      } else {
      ...
      if (!this.selectData.length) {
        ...
        noInterfaceDelete.forEach((val) => {
          this.tableListData = this.tableListData.filter((item) => {
            if (item.cartypeConfigId !== val.cartypeConfigId) return item;
          });
        });
        if (idsLsit.length != 0) {
    ...
    fillDown() {
      this.$nextTick(() => {
        ...
        if (!this.selectData[0].perCarDosage)
          return iMessage.warn(
            this.language("QINGSHURUMEICHEYONGLIANG", "请输入每车用量")
          );

        let afterFoucsRowFlag = false;
        this.tableListData.forEach((item) => {
          if (afterFoucsRowFlag)
            this.$set(item, "perCarDosage", this.selectData[0].perCarDosage);
    ...
    saveData() {
      ...
      if (flag == false) {
        ...
        savearDosage(this.tableListData)
          .then((res) => {
            if (res.code == "200") {
