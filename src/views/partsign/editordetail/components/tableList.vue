 ...
 * @LastEditTime: 2021-06-25 11:08:30
...
<template>
  <el-table class="table" :height="height" :data="tableData" :cell-class-name="cellClassName" v-loading="tableLoading" @selection-change="handleSelectionChange" :empty-text="$t('LK_ZANWUSHUJU')">
    ...
    <template v-for="(item, $index) in tableTitle">
      <el-table-column :key="$index" align="center" :label="item.lang ? language(item.key, item.name) : $t(item.key)" :prop="item.props" :show-overflow-tooltip="item.tooltip" :width="item.width">
