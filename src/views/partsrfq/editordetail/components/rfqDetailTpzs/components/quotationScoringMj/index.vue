...
export default {
  ...
  created() {
    ...
    this.partInfo.currentRounds = parseInt(
      this.getbaseInfoData().currentRounds
    );
  ...
  methods: {
    ...
    handleChangeByAssembledPartCode(partNum, row) {
      ...
      const mouldIdIndexes = this.tableListData
        .filter((item) => item.priceType === row.priceType)
        .map((item) => {
      ...
      if (row.assembledPartPrjCode && row.priceType) {
        this.$set(
          row,
          "mouldId",
          `${this.partInfo.rfqId}_${this.supplierId}_${
            row.assembledPartPrjCode
          }_${row.priceType === "LC" ? "TL" : "TS"}${index}`
        );
      }
    },
    // fs号选择--重写父方法。供应商ID
      ...
      if (!row.assembledPartCode) {
        ...
        if (fsNum && row.priceType) {
          this.$set(
            row,
            "mouldId",
            `${this.partInfo.rfqId}_${this.supplierId}_${fsNum}_${
              row.priceType === "LC" ? "TL" : "TS"
            }${index}`
          );
        }
      }
