  ...
  <div>
    <div class="right-btn">
      ...
      <iButton
        @click="handleReport"
        v-permission.auto="
          RFQ_DETAIL_TIPS_NEGOTIATEBASIC_BUTTON_BAOGAOQINGDAN | 报告清单
        "
        >{{ $t("TPZS.BGQD") }}</iButton
      >
    </div>
    ...
    <iCard
      ...
      collapse
      v-permission.auto="
        RFQ_DETAIL_TIPS_BAOJIAYUPINGFENGENZONG | 报价与评分跟踪
      "
    >
      <quotationScoringTracking
        v-if="cardShow.find((items) => items.key == '1').show"
      ></quotationScoringTracking>
    </iCard>
    <!--------------------报价分析汇总-零件----------------------------------------->
    <iCard
      ...
      v-permission.auto="
        RFQ_DETAIL_TIPS_BAOJIAFENXIHUIZONGLINGJIAN | (报价分析汇总 - 零件)
      "
    >
      <quotationScoringHZ
        :rfqInfoData="rfqInfoData"
        ref="quotationScoringHZ"
        v-if="cardShow.find((items) => items.key == '2').show"
      ></quotationScoringHZ>
    </iCard>
    <!--------------------报价分析汇总-模具----------------------------------------->
    <quotationScoringMj
      :rfqInfoData="rfqInfoData"
      ref="quotationScoringMj"
    ...
    <iCard
      @handleTitle="addFile($event, 3, '报价分析汇总-模具', 1)"
      :title="'报价分析汇总-模具'"
      v-else
      class="margin-top20"
      @handleCollapse="handleCollapse($event, '3')"
      ...
      v-permission.auto="
        RFQ_DETAIL_TIPS_BAOJIAFENXIHUIZONGMOJU | (报价分析汇总 - 模具)
      "
    ></iCard>
    ...
    <quotationScoringEcartsCard
      :rfqInfoData="rfqInfoData"
      id="card4"
      ref="quotationScoringEcartsCard"
      v-if="cardShow.find((items) => items.key == '4').show"
    ></quotationScoringEcartsCard>
    <iCard
      @handleTitle="addFile($event, 4, '报价趋势', 1)"
      ...
      collapse
      :defalutCollVal="false"
      v-permission.auto="RFQ_DETAIL_TIPS_BAOJIAQUSHI | 报价趋势"
    ></iCard>
    <!--------------------报价分析汇总-业务分配模拟----------------------------------------->
    <buMonitor
      @handleCollapse="handleCollapse($event, '5', '业务分配模拟')"
      id="card5"
      :collapse="true"
      :rfqInfoData="rfqInfoData"
      :hideCombine="false"
      ...
      class="margin-top20"
      v-permission.auto="RFQ_DETAIL_TIPS_YEWYFENPEIMONI | 业务分配模拟"
    ></buMonitor>
    <iCard
      @handleTitle="addFile($event, 5, '业务分配模拟')"
      id="card5"
      :title="'业务分配模拟'"
      v-else
      ...
      collapse
      :defalutCollVal="false"
      v-permission.auto="RFQ_DETAIL_TIPS_YEWUFENPEIMONI | 业务分配模拟"
    ></iCard>
  </div>
</template>
<script>
