        ...
        <el-table-column
          ...
          width="50">
				...
				>
					<template slot-scope="scope">
						<el-row type="flex" align="middle">
							...
							<el-col :span="4" >
								<iNumIcon style="margin-top: 20px;" v-if="scope.row.reportList" :num="(scope.row.reportList!==null)?scope.row.reportList.length:0"/>
							</el-col>
						</el-row>
					</template>
				</el-table-column>
            ...
            <div @click="handleStick(scope.row)" class="stickIcon">
						  <icon v-if="scope.row.fileType==='方案' && scope.row.isTop" name="iconliebiaoyizhiding"  class="iconliebiaoyizhiding" symbol/>
						  <icon v-else-if="scope.row.fileType==='方案' && !scope.row.isTop" name="iconliebiaoweizhiding"  class="iconliebiaoweizhiding" symbol/>
            </div>
					</template>
				</el-table-column>
		</el-table>
    ...
    // 选中项发生改变
      ...
      this.selection = val
		...
		checkAll(currentSelect){
			const haschild =currentSelect.filter((r)=>r.reportList&&r.reportList!==null&&r.reportList.length>0)
			if(haschild.length>0){
				currentSelect.map((r)=>{
					if(r.reportList&&r.reportList!==null&&r.reportList.length>0){
						r.reportList.forEach((c)=>{
		...
		checkChildren(currentSelect,row){
			...
			if(row.reportList&&row.reportList!==null&&row.reportList.length>0) {
