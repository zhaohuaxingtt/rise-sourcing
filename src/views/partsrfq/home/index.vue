          ...
          <iCard>
            ...
            <div class="margin-bottom20 clearFloat">
              ...
              <div class="floatright">
                ...
                <iButton @click="editRfq('02')" :loading="activateButtonLoading" v-permission="PARTSRFQ_ACTIVATERFQ">{{$t('rfq.RFQACTIVATERFQ')}}</iButton>
                ...
                <iButton @click="editRfq('01')" :loading="closeButtonLoading" v-permission="PARTSRFQ_CLOSERFQ">{{$t('rfq.RFQCLOSERFQ')}}</iButton>
                <!--转派评分任务：选中RFQ之后，可以手动转派任务给EP/MQ同事-->
                <iButton @click="assignmentOfScoringTasks" v-permission="PARTSRFQ_ASSIGNMENTTASKS">{{$t('rfq.RFQASSIGNMENTOFSCORINGTASKS')}}</iButton>
                <!--转谈判：只会出现在前期采购员界面-->
                <iButton @click="editRfq('03')" :loading="transferNegotiationButtonLoading" v-permission="PARTSRFQ_TRANSFERNEGOTIATION">{{$t('rfq.RFQTRANSFERNEGOTIATION')}}</iButton>
                <!--转询价：只会出现在专业采购员界面-->
                <iButton @click="editRfq('04')" :loading="transferInquiryButtonLoading" v-permission="PARTSRFQ_REINQUIRY">{{$t('rfq.RFQREINQUIRY')}}</iButton>
                <!--创建定点申请：在列表中选择RFQ，点击该键，会跳转到定点申请创建页面，RFQ的内容会自动带入到定点申请的各页签中-->
                <iButton disabled v-permission="PARTSRFQ_CREATEAPPLICATION">{{$t('rfq.RFQCREATEAFIXEDPOINTAPPLICATION')}}</iButton>
