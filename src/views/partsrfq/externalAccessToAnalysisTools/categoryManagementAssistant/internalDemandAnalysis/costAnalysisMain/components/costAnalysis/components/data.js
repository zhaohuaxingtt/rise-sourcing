/*
 * @Author: youyuan
 * @Date: 2021-08-02 18:07:18
 * @LastEditTime: 2021-08-02 18:12:15
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \front-web\src\views\partsrfq\externalAccessToAnalysisTools\categoryManagementAssistant\internalDemandAnalysis\components\costAnalysisMain\components\costAnalysis\components\data.js
 */

export const tableTitle = [
  {props: 'analysis', name: '分析名称', key: 'FENXIMINGCHENG'},
  {props: 'materialGroup', name: '材料组', key: 'CAILIAOZU'},
  {props: 'createBy', name: '创建人', key: 'CHUANGJIANREN'},
  {props: 'createDate', name: '创建日期', key: 'CHUANGJIANRIQI'},
  {props: 'option', name: '操作', key: 'CAOZUO'},
  {props: 'lastUpdateDate', name: '上次修改日期', key: 'SHANGCIXIUGAIRIQI'},
  {props: 'sort', name: '', key: 'SORT'},
]
