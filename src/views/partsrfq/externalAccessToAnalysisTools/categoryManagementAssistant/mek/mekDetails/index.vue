 ...
 * @LastEditTime: 2021-08-27 15:00:04
 * @LastEditors: zbin
    ...
    <div id="content">
      <div class="navBox flex-between-center">
        <div class="title font-weight flex">
          ...
          <iSelect @change="changeCategory" v-model="categoryCode">
            <el-option v-for="item in categoryList" :key="item.categoryId" :value="item.categoryCode" :label="item.categoryName">
            </el-option>
          </iSelect>
        </div>
        <div class="flex">
          ...
          <!--保存-->
          <iButton @click="handleMEKInfo" class="margin-left30">{{ $t("MEK基础数据库") }}</iButton>
          <!--BoB分析库-->
          <iButton class="margin-left30">{{ $t("预览") }}</iButton>
          <!--查找零件-->
          <iButton class="margin-left30" @click="saveDialog">{{ $t("保存") }}</iButton>
          <icon symbol name="iconrizhi" class="margin-left30 "></icon>
        </div>
      </div>
      <div class=" bodyBox clearFloat">
        <div class="cardBox" style="width:18%">
          <iCard>
            <div class=" searchForm" style="margin-right:-20px">
              <el-form label-position="top" :model="form" style="height:504px;margin-right:10px" label-width="200px">
                <el-row class="margin-bottom20">
                  <!--对标车型-->
                  <el-form-item :label="$t('对标车型')" label-width="180px">
                    <el-select v-model="ComparedMotor" @change="changeComparedMotor" multiple>
                      <el-option v-for="item in ComparedMotorList" :key="item.motorId" :value="item.motorId" :label="item.motorName"> </el-option>
                    </el-select>
                  </el-form-item>
                  <!--类型选择-->
                  <el-form-item :label="$t('类型选择')">
                    <el-select v-model="comparedType">
                      <el-option v-for="item in mekTypeList" :key="item.id" :value="item.code" :label="item.name"> </el-option>
                    </el-select>
                  </el-form-item>
                  <!--零件六位号-->
                  <el-form-item :label="$t('零件六位号')">
                    <el-select v-model="partNumber" multiple>
                      <el-option v-for="item in recursiveRetrieveList" :key="item.partId" :value="item.partSixNumber" :label="item.partNumber"> </el-option>
                    </el-select>
                  </el-form-item>
                </el-row>
              </el-form>
            ...
            <div class="end">
              <iButton type="primary" style="width:100px;height:35px" @click="searchChartData">{{ $t("LK_QUEDING") }}</iButton>
              <iButton type="primary" style="width:100px;height:35px" @click="handleSearchReset">{{ $t("LK_ZHONGZHI") }}</iButton>
            </div>
          </iCard>
        </div>
        <div class="cardBox" style="width:82%">
          <iCard class="margin-left20" style="height:619px">
            <div class="chartBox1">
              <div class="line"></div>
              <div class="line1"></div>
              <div class="line2"></div>
              ...
              <div class="line4"></div>
              <div class="chartBox">
                <div class="flex chartItem" :style="{width:chartItemWidth}">
                  <div class="operation1">
                    <el-select v-model="targetMotor" @change="changeTargetMotor" style="width:150px" class="margin-bottom15 margin-top40" placeholder="请选择目标车型">
                      <el-option v-for="item in TargetMotorList" :key="item.motorId" :value="item.motorId" :label="item.motorName"> </el-option>
                    </el-select>
                    <span class="margin-bottom15 " style="min-height:14px">{{firstBarData.motorName}}</span>
                    <span class="yield" style="line-height:12px">{{firstBarData.output}}</span>
                  </div>
                  <datasetBar1 ref="datasetBar1" :typeSelection="mekMotorTypeFlag" :firstBarData="firstBarData.detail" :maxWidth="maxWidth" @detailDialog="detailDialog"></datasetBar1>
                  <div class="xAxis" v-if="mekMotorTypeFlag">
                    <div v-for="i in firstBarData.detail" :key="i.value" style="text-align:center">
                      <div style="margin-bottom:10px">
                        <span @click="computeModal(firstBarData)">{{i.title}}</span>
                        <el-tooltip class="item" effect="dark" :content="firstBarData.tips" placement="top-end">
                          <icon name="iconzengjiacailiaochengben_lan" symbol style="width:14px;height:14px;margin-left:10px"></icon>
                        </el-tooltip>
                      </div>
                ...
                <div class="flex chartItem" :style="{width:chartItemWidth}" v-for="item in barData" :key="item.motorId">
                  <div class="operation">
                    <icon symbol name="iconbob-shanchu" class="margin-bottom20 " style="width:20px;height:20px"></icon>
                    <el-popover placement="bottom" width="80" trigger="click" visible-arrow class="margin-bottom15">
                      <el-checkbox-group v-model="item.checkList" class="checkList" @change="changeCheckList">
                        <el-checkbox v-for="(i,index) in item.detail" :key="index" :label="i.value">{{i.title}}</el-checkbox>
                      </el-checkbox-group>
                      <div style="line-height:30px" slot="reference">{{item.motorName}}</div>
                    </el-popover>
                    <span class="margin-bottom15" style="line-height:16px;height:16px">{{item.factory}}</span>
                    <span class="yield margin-bottom15">{{item.output}}</span>
                    <div>
                      <el-select v-model="item.priceType" @change="changPriceType" style="width:150px;z-index:1000" v-if="flag1">
                        <el-option v-for="i in mekpriceTypeList" :key="i.id" :value="i.code" :label="i.name"> </el-option>
                      </el-select>
                      <el-date-picker v-model="date" type="date" placeholder="选择日期" @change="changeDate" style="width:150px;z-index:1000" v-if="priceType==='2'">
                      </el-date-picker>
                    </div>
                  </div>
                  <datasetBar :barData="item" :maxWidth="maxWidth" :typeSelection="mekMotorTypeFlag" @detailDialog="detailDialog"></datasetBar>
                  <div class="xAxis" v-if="mekMotorTypeFlag">
                    <div v-for="i in item.detail" :key="i.value" style="text-align:center">
                      <div style="margin-bottom:10px">
                        <span @click="computeModal(item)">{{i.title}}</span>
                        <el-tooltip class="item" effect="dark" :content="item.tips" placement="top-end">
                          <icon name="iconzengjiacailiaochengben_lan" symbol style="width:14px;height:14px;margin-left:10px"></icon>
                        </el-tooltip>
                      </div>
      ...
      <tableList :gridData="gridData" :editFlag="editFlag" :addRowList="addRowList" @editData="editData" @addData="addData"></tableList>
      ...
      <modalDialog :modalVisible="modalVisible" @input="closeModalDialog" :computeModalData="computeModalData"></modalDialog>
      <detailDialog :detailVisible="detailVisible" :detailsData="detailsData" @input="closeModalDialog1"></detailDialog>
    </div>
  </iPage>
</template>
...
import { getMekTable, getHistogram, category, getComparedMotor, getTargetMotor, recursiveRetrieve, saveMekTable, deleteMekTable, getSchemeInfo, queryPartEbr, queryCal } from '@/api/categoryManagementAssistant/mek'
