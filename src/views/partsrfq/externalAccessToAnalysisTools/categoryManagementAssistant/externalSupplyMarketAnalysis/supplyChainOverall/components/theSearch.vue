...
import { userCategory, getCityInfo, nTierParamInit } from "@/api/partsrfq/supplyChainOverall/index.js";
...
export default {
  ...
  methods: {
    handleSave() {
      this.$parent.handleSave(this.form, this.id)
    },
    async getInit() {
      const res = await nTierParamInit({ categoryCode: this.$store.state.rfq.categoryCode })
      this.id = res.data.id
      if (res.data.ntierQueryConditionDTO) {
        this.form = res.data.ntierQueryConditionDTO
      }
    },
    getMapList() {
