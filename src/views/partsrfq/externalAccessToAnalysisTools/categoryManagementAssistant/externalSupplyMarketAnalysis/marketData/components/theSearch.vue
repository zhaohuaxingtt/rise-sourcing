  ...
  <div class="searchBox">
    <div v-for="(item,index) of list"
      ...
      <template v-if="item.type === 'select'">
        <iSelect v-model="form[item.props]"
          ...
          <el-option :value="item2"
                     :label="item2"
                     v-for="item2 of item.options"
                     :key="item2"></el-option>
        </iSelect>
      </template>
      <template v-else-if="item.type === 'dateMonth'">
        <el-date-picker v-model='form[item.props]'
      ...
      <template v-else-if="item.type === 'input'">
        <iInput v-model="form[item.props]"
                :placeholder="item.placeholder" />
      </template>
      <template v-else-if="item.type === 'selectCustom'">
        <iSelectCustom v-model="form[item.props]"
                       :data="item.options"
                       :multiple="true"
                       label="name"
                       sortVal="name"
      ...
      <template v-else-if="item.type === 'selectCascader'">
        <el-cascader v-model="form[item.props]"
