...
<template>
  <div class="aekodetail">
    ...
    <iCard :title="language('LK_JICHUXINXI', '基础信息')">
      <iFormGroup row="4" class="basic-form" label-width="100px">
        <template v-for="(item, index) in basicTitle">
          ...
          >
            <iText v-if="item.isObj">{{
              aekoInfo[item.props] && aekoInfo[item.props]["desc"]
            }}</iText>
    ...
    <iTabsList
      class="aekodetail-tabs margin-top20"
      type="card"
      v-model="currentTab"
      @tab-click="tabChange"
    >
      ...
      <el-tab-pane
        ...
        :label="tab.label"
        :name="tab.name"
        v-permission.dynamic="tab.permissionKey"
      >
        <span slot="label"
          >{{ tab.label
          }}<icon
...
import { aekoAuditSupplementalresult } from "@/api/aeko/detail/approveRecord";
...
export default {
  ...
  async created() {
    ...
    } else if ((auditType && auditType == 1) || auditType == 2) {
      //封面表态
      this.currentTab = "cover";
    } else if (auditType && auditType == 3) {
      // 内容表态
      this.currentTab = "contentDeclare";
    }
    ...
    if (from == "manage") {
      ...
      newTabs = newTabs.filter(
        (item) =>
          item.name !== "record" &&
          (item.name !== "attachment") & (item.name !== "contentDeclare")
      );
      this.tabs = newTabs;
  ...
  data() {
    return {
      ...
      basicTitle: [
        {
          label: "AEKO状态",
          labelKey: "LK_AEKOZHUANGTAI",
          props: "aekoStatusDesc",
          permissionKey: "AEKO_AEKODETAIL_TEXT_STATUS|AKEO详情-AEKO状态",
        },
        {
          label: "来源",
          labelKey: "LK_AEKO_LAIYUAN",
          props: "sourseDesc",
          permissionKey: "AEKO_AEKODETAIL_TEXT_SOURCE|AKEO详情-来源",
        },
        {
          label: "创建⽇期",
          labelKey: "LK_AEKOCHUANGJIANRIQI",
          props: "createDate",
          permissionKey: "AEKO_AEKODETAIL_TEXT_CREATE_DATE|AKEO详情-创建⽇期",
        },
        {
          label: "截⽌⽇期",
          labelKey: "LK_AEKOJIEZHIRIQI",
          props: "deadLine",
          permissionKey: "AEKO_AEKODETAIL_TEXT_DUE_DATE|AKEO详情-截⽌⽇期",
        },
      ],
      tabs: [
        {
