 ...
 * @LastEditTime: 2021-11-12 12:15:08
 * @LastEditors: YoHo
 * @Description: 
-->
<template>
      ...
      await getCbdkent({
        ...
        if(res.code == 200){
          if(res.code == 200){
            ...
            this.$set(this.dataGroup, "devFee", res.data.totalPrice) // 开发费合计
            this.$set(this.dataGroup, "shareDevFee", res.data.shareTotal) // 分摊开发费用
            this.$set(this.dataGroup, "shareQuantity", res.data.shareQuantity) // 分摊数量
            this.$set(this.dataGroup, "unitPrice", res.data.shareAmount) // 单件开发成本
          }
        }else{
