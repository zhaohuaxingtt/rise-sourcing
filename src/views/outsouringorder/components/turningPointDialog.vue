  ...
  <iDialog :title="$t('LK_QIANSHOU')" :visible.sync="value" width="25%" @close="clearDiolog" class="point-dialog">
    <el-form>
      ...
      <el-form-item :label="$t('MOULDADD.LK_QIANQICAIGOUYUAN')">
        <iSelect :placeholder="$t('LK_QINGXUANZE')" v-model="buyer" value-key="id">
          <el-option v-for="(items, index) in userList" :key="index" :value="items" :label="items.nameZh" />
        </iSelect>
      </el-form-item>
...
import { getDepartmentsCombo, liniePullDownByDept } from '@/api/ws2/purchase/investmentList'
