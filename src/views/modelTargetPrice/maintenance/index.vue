...
<template>
  <iPage v-permission.auto='MODELTARGETPRICE_MAINTENANCE_PAGE|模具目标价管理-目标价维护-页面'>
    ...
    <iCard class="margin-top20" v-permission.auto='MODELTARGETPRICE_MAINTENANCE_TABLE|模具目标价管理-目标价维护-表格'>
      <div class="margin-bottom20 clearFloat">
        ...
        <div class="floatright">
          <!--------------------指派按钮----------------------------------->
          <iButton @click="openAssignDialog" :loading="assignDialogVisible" v-permission.auto='MODELTARGETPRICE_MAINTENANCE_ASSIGN|模具目标价管理-目标价维护-指派'>{{language('ZHIPAI','指派')}}</iButton>
        </div>
      </div>
      <tableList 
        ref="tableList"
    ...
    <!--                  指派弹窗                                      --->
    <!------------------------------------------------------------------------>
    <assignDialog ref="assignDialog" :dialogVisible.sync="assignDialogVisible" @changeVisible="changeAssignDialogVisible" @sendAccessory="targetAppoint" />
  </iPage>
</template>
...
export default {
  mixins: [pageMixins],
  components: {carProjectSelect,iDicoptions,procureFactorySelect,iPage,headerNav,iCard,tableList,iPagination,iButton,iSelect,iDatePicker,iInput,iSearch,attachmentDialog,approvalRecordDialog, assignDialog, iMultiLineInput},
  data() {
    return {
      tableTitle: tableTitle,
      tableData: [],
