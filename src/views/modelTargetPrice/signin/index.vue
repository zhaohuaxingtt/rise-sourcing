...
<template>
  <iPage v-permission.auto='MODELTARGETPRICE_SIGNIN_PAGE|模具目标价管理-目标价签收-页面'>
    ...
    <iCard class="margin-top20" v-permission.auto='MODELTARGETPRICE_SIGNIN_TABLE|模具目标价管理-目标价签收-表格'>
      <div class="margin-bottom20 clearFloat">
        ...
        <div class="floatright">
          ...
          <!--------------------无投资按钮----------------------------------->
          <iButton @click="handleNoInvest" :loading="noInvestLoading" v-permission.auto='MODELTARGETPRICE_SIGNIN_NOINVESTBTN|模具目标价管理-目标价签收-无投资按钮'>{{language('WUTOUZI','无投资')}}</iButton>
          <!--------------------签收按钮----------------------------------->
    ...
    <!--                  退回弹窗                                      --->
    <!------------------------------------------------------------------------>
    <sendBackDialog ref="sendBackConfirm" :dialogVisible="sendBackDialogVisible" @changeVisible="changeSendBackDialogVisible" @handleConfirm="handleSendBackConfirm"  />
  </iPage>
</template>
...
export default {
  ...
  components: {carProjectSelect,procureFactorySelect,iDicoptions,iPage,headerNav,iCard,tableList,iPagination,iButton,iSelect,iDatePicker,iInput,iSearch,attachmentDialog, assignDialog, sendBackDialog, noInvestConfirmDialog, iMultiLineInput},
  data() {
    return {
      tableTitle: tableTitle,
      tableData: [],
