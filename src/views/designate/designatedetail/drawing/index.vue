<template>
  <iCard
    class="designate-drawing"
    v-permission.auto="
      SOURCING_NOMINATION_ATTATCH_DRAWING | (决策资料 - drawing)
    "
  >
    <div class="margin-bottom25 clearFloat">
      ...
      <div class="floatright" v-if="!$store.getters.isPreview">
        ...
        <iButton
          ...
          @click="sortVisibal = true"
          v-permission.auto="SOURCING_NOMINATION_ATTATCH_DRAWING_SORT | 排序"
        >
        ...
        <iButton
          ...
          v-permission.auto="
            SOURCING_NOMINATION_ATTATCH_DRAWING_DOWNLOADALL | 全部下载
          "
        >
        ...
        <iButton
          v-if="!nominationDisabled && !rsDisabled"
          @click="uploadVisibal = true"
          v-permission.auto="SOURCING_NOMINATION_ATTATCH_DRAWING_UPLOAD | 上传"
        >
    ...
    <div
      class="list"
      :class="{ 'data-null': !(dataList && dataList.length) }"
      ...
      <el-row gutter="20" v-if="dataList && dataList.length">
        ...
        >
          <div class="drawing-content">
            <img
              v-if="
                ['.jpg', '.jpeg', '.png', '.bmp', '.webp'].some((type) =>
                  String(item.fileName).toLowerCase().endsWith(type)
                )
              "
              class="img-preview"
              :src="item.filePath"
              @click="openPhotoList(item)"
            />
            <span v-else
              ><i class="el-icon-document"></i>{{ item.fileName }}</span
            >
            <a
              class="trigger"
              href="javascript:;"
              @click="dowloadSingleFile(item)"
              v-permission.auto="
                SOURCING_NOMINATION_ATTATCH_DRAWING_DOWNLOADSINGLE | 图纸下载
              "
            >
    ...
    <PhotoList
      :imgList="imgList"
      :visible="photoListShow"
      @changeLayer="() => (photoListShow = false)"
    />
  </iCard>
</template>
<script>
