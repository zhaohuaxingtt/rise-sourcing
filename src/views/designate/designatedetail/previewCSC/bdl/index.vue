 ...
 * @LastEditTime: 2023-02-01 16:50:54
...
<div ref="bdl">
  ...
  <iCard v-for="(item, index) in rfqList" :key="index" :title="'RFQ NO.'+(item.rfqNum||'-')+',RFQ Name:'+(item.rfqName||'-')" class="margin-top20">
    <template v-if="item.tableTitle && item.tableData">
    <tableList :tableRowClassName="'table-row'+index" :tableTitle="item.tableTitle" :selection="false" index :tableData="item.tableData" class="doubleHeader" @openDialog="openRateDialog($event, item.rfqNum)" v-permission.auto="SOURCING_NOMINATION_ATTATCH_BDL_TABLE|决策资料-bdl-表格">
      
      <template #supplierName="scope">
        <div>
          <span class="factoryDesc">{{scope.row.supplierName }}</span> <span v-if="!scope.row.isMbdl">MBDL</span>
          ...
          />
          <div><span>{{ scope.row.sapCode }}</span> {{ scope.row.supplierNameEn }}</div>
        </div>
      </template>
      <template #isQuotation="scope">
        <icon v-if="scope.row.isQuotation" symbol name="iconxialakuang_qiehuanlingjian_yiwancheng" />
      </template>
      <template #isPartQuotation="scope">
        <icon v-if="scope.row.isPartQuotation" symbol name="iconxialakuang_qiehuanlingjian_yiwancheng" />
      </template>
      <template #isRefuse="scope">
        <icon v-if="scope.row.isRefuse" symbol name="iconxialakuang_qiehuanlingjian_yiwancheng" />
      </template>
      <template #noQuotation="scope">
        <icon v-if="scope.row.noQuotation" symbol name="iconxialakuang_qiehuanlingjian_yiwancheng" />
      </template>
    </tableList>
