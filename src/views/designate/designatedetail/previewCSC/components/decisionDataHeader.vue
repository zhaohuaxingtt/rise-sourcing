...
<template>
  <div
    :class="
      isPreview == '1' ? 'decision-header preview-header' : 'decision-header'
    "
  >
    ...
    <span
      class="tab-icon"
      v-if="isPreview == '1' && !$route.meta.hideCloseButton"
      @click="close"
    >
    ...
    <span
      class="tab-icon"
      @click="sortDialogVisibal = true"
      v-else-if="!isTemp && isPreview !== '1'"
      v-permission.auto="
        NOMINATION_NOMINATIONDETAILS_MOUDULES_SORT | 定点管理详情菜单排序
      "
      ><icon symbol name="iconSetting"></icon
    ></span>
    <!-- 排序弹窗 -->
...
<script>
import { iTabsList, icon, iButton } from "rise";
import { updatePresenPageSeat } from "@/api/designate";
...
export default {
  ...
  mounted() {
    ...
    setTimeout(() => {
      // 临时跳转的时候，直接取前端写死的配置
      if (this.isTemp || this.$route.meta.layoutPath)
        this.decisionType = decisionType;
      this.defaultTab = path;
      // 配置了layoutPath，用于预览，在数据定义中找用于选中的path地址
      if (this.$route.meta.layoutPath) {
        const defaulTab = decisionType.find((o) =>
          String(o.path).includes(this.$route.meta.path)
        );
