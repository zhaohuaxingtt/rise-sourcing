...
<template>
  <iPage>
    ...
    <iSearch
      ...
      @sure="filterTableData"
    >
      <el-form>
        <el-form-item
          :label="language('LINGJIANCAIGOUXIANGMUBIANHAO', '零件采购项目编号')"
        >
          <iInput
    ...
    <iCard>
      <div class="margin-bottom20 clearFloat">
        <div class="floatright">
          ...
          <iButton
            ...
            :loading="saveLoading"
            v-permission.auto="
              RSSINGLEMAINTENANCE_SAVEBTN | (RS单维护 - 保存按钮)
            "
          ...
          <iButton
            ...
            v-permission.auto="
              RSSINGLEMAINTENANCE_DOWNLOADTEMPLATEBTN |
                (RS单维护 - 下载模板按钮)
            "
            >{{ language("XIAZAIMOBAN", "下载模板") }}</iButton
          >
          <!--------------------返回按钮----------------------------------->
          ...
          <el-upload
            ...
            :on-error="
              () => {
                uploadLoading = false;
                iMessage.error(language('SHANGCHUANSHIBAI', '上传失败！'));
              }
            "
            ...
            v-permission.auto="
              RSSINGLEMAINTENANCE_UPLOADBTN | (RS单维护 - 上传按钮)
            "
          >
          ...
          <iButton
            v-if="!nominationDisabled && !rsDisabled"
            @click="handleReadQuotation"
            :loading="readQuotationLoading"
            v-permission.auto="
              RSSINGLEMAINTENANCE_READQUOTATIONBTN | (RS单维护 - 读取报价单按钮)
            "
            >{{ language("DUQUBAOJIADAN", "读取报价单") }}</iButton
          >
          <!--------------------RS单预览按钮----------------------------------->
          <iButton @click="handlePreviewRS">{{
            language("RSDANYULAN", "RS单预览")
          }}</iButton>
        </div>
      </div>
      <!------------------------------------------------------------------------>
      ...
      <!------------------------------------------------------------------------>
      <tableList
        :disabled="nominationDisabled || rsDisabled"
        :activeItems="'rfqId'"
        selection
        indexKey
    ...
    <rsDialog
      :dialogVisible="rsEeditionDialogVisible"
      @changeVisible="changersEeditionDialogVisible"
      :otherPreview="false"
      :otherNominationType="otherNominationType"
      :otherNominationId="otherNominationId"
      :otherPartProjectType="otherPartProjectType"
    />
  </iPage>
</template>
    ...
    tableTitle() {
      if (
        [partProjTypes.GSLINGJIAN, partProjTypes.GSCOMMONSOURCING].includes(
          this.partProjectType
        )
      ) {
        return rsTableTitle;
      }
      return rsTableTitle.filter(
        (item) => !["addFee", "savingFee", "presentPrice"].includes(item.props)
      );
    },
  },
  created() {
  ...
  methods: {
      ...
      nominateAppSDetail({
        ...
        if (res.code == 200) {
          this.$store.dispatch(
            ...
            getNominateDisabled(
              { ...res.data, designateType: this.$route.query.designateType } ||
                {}
            )
          );
    ...
    async downloadTemp() {
      if (this.tableListData.length < 1) {
        iMessage.warn(
          this.language("QINGXUANZEXUYAOXIAZAIDESHUJU", "请选择需要下载的数据")
        );
        return;
      }
    ...
    handleReadQuotation() {
      if (this.selectedTableData.length < 1) {
        iMessage.warn(
          this.language(
            "QINGXUANZEXUYAODUQUDEBAOJIADAN",
            "请选择需要读取的报价单"
          )
        );
        return;
      ...
      const params = {
        nominateAppId: this.$route.query.desinateId,
        nomiMaintenanceRecordIdlist: this.selectedTableData.map(
          (item) => item.nomiMaintenanceRecordId
        ),
      };
          ...
          ltcs: defaultLtcs.map((ltcsItem, ltcIndex) => {
            return {
              ltcDate: item["ltcDate" + (ltcIndex + 1)]
                ? moment(item["ltcDate" + (ltcIndex + 1)]).format("yyyy-MM")
                : "",
    ...
    getTableList() {
      ...
      manualListRs(this.$route.query.desinateId)
        .then((res) => {
          if (res?.result) {
            ...
            const cloneData = cloneDeep(res.data?.lines).map((item) => {
              ...
              watchChangeData?.forEach((element, index) => {
                singleItem[element + "Temp"] = cloneDeep(
                  item[element] === null ? "" : item[element]
                );
              });
              defaultLtcs?.forEach((element, index) => {
                (singleItem["ltcDate" + (index + 1)] = cloneDeep(
                  item.ltcs && item.ltcs[index]?.ltcDate
                    ? item.ltcs[index].ltcDate
                    : element.ltcDate
                )),
                  (singleItem["ltcDate" + (index + 1) + "Temp"] = cloneDeep(
                    item.ltcs && item.ltcs[index]?.ltcDate
                      ? moment(item.ltcs[index].ltcDate).format("yyyy-MM")
                      : element.ltcDate
                  )),
                  (singleItem["ltcDateIsChange" + (index + 1)] =
                    item.ltcs && item.ltcs[index]?.ltcDateIsChange
                      ? item.ltcs[index].ltcDateIsChange
                      : element.ltcDateIsChange),
                  (singleItem["ltcRate" + (index + 1)] = cloneDeep(
                    item.ltcs && item.ltcs[index]?.ltcRate
                      ? item.ltcs[index].ltcRate
                      : element.ltcRate
                  )),
                  (singleItem["ltcRate" + (index + 1) + "Temp"] = cloneDeep(
                    item.ltcs && item.ltcs[index]?.ltcRate
                      ? item.ltcs[index].ltcRate
                      : element.ltcRate
                  )),
                  (singleItem["ltcRateIsChange" + (index + 1)] =
                    item.ltcs && item.ltcs[index]?.ltcRateIsChange
                      ? item.ltcs[index].ltcRateIsChange
                      : element.ltcRateIsChange);
              });
              return singleItem;
            });
