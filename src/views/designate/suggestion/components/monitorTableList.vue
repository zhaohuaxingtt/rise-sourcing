 ...
 * @LastEditTime: 2021-06-25 16:59:16
          ...
          <template slot-scope="scope">
            <div v-if="batchEdit">
              <iInput
                ...
                :placeholder="scope.row.percent && scope.row.percent[hindex - 1] || language('LK_QINGSHURU')"
  ...
  methods: {
    ...
    handleEditPercent(row, Index) {
      ...
      if (isNaN(count) || count > 100 || containNGNumber) {
        // percent[Index] = 0
        // Vue.set(row, 'percent', percent)
        iMessage.error(this.language('nominationSuggestion.NingShuRuDeBiLiBuHeFa'))
        return
      }
      this.onPercentChangeWhiteBack(row)
      this.chartData = this.calculateBestTTo(this.data)
