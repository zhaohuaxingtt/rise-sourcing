...
<template>
  <div class="datamaintenance">
    ...
    <iCard class="card"
      ...
      <template v-slot:header-control>
        ...
        <iButton @click="handleExport"
                 v-permission.auto="COSTANALYSISMANAGE_DATAMAINTENANCE_COSTMAINTENANCE_BUTTON_EXPORTTEMPLATE|导出模板">{{ language("DAOCHUMUBAN", "导出模板") }}</iButton>
        <iButton class="marginRight10"
                 @click="openDate"
                 :loading="uploadLoading"
                 v-permission.auto="COSTANALYSISMANAGE_DATAMAINTENANCE_COSTMAINTENANCE_BUTTON_UPLOAD|上传">{{ language("SHANGCHUAN", "上传") }}</iButton>
        <iButton @click="handleDownload"
                 v-permission.auto="COSTANALYSISMANAGE_DATAMAINTENANCE_COSTMAINTENANCE_BUTTON_DOWNLOAD|下载文件">{{ language("XIAZAIWENJIAN", "下载文件") }}</iButton>
        <iButton :loading="deleteLoading"
                 @click="handleDelete"
                 v-permission.auto="COSTANALYSISMANAGE_DATAMAINTENANCE_COSTMAINTENANCE_BUTTON_DELETE|删除">{{ language("SHANCHU", "删除") }}</iButton>
      </template>
      <div class="body">
        <tableList class="table"
          ...
          <template #fileName="scope">
            <span class="link-underline"
                  @click="download(scope.row)">{{ scope.row.fileName }}</span>
          </template>
    ...
    <!-- 选择时间 -->
    <dateSelect v-model="dateValue"
                @sure="sure"></dateSelect>
  </div>
</template>
