/*
 * @Author: your name
 * @Date: 2021-06-18 10:40:28
 * @LastEditTime: 2021-06-18 17:32:44
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \front-web\src\views\configgradedept\components\data.js
 */
export const queryForm = {}

export const tableTitle = [
  { props: "deptScoreType", name: "部门评分类型", key: "部门评分类型", tooltip: true },
  { props: "deptNum", name: "部门编号", key: "部门编号", tooltip: true },
  { props: "isAudit", name: "是否审核", key: "是否审核", tooltip: true },
]

export const deptDialogQueryForm = {}

export const deptDialogTableTitle = [
  { props: "deptNum", name: "部门编号", key: "部门编号", tooltip: true },
  { props: "deptNameZh", name: "部门中文名", key: "部门中文名", tooltip: true },
  { props: "deptNameEn", name: "部门英文名", key: "部门英文名", tooltip: true },
]