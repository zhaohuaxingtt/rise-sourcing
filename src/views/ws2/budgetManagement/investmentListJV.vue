	...
	<div class="investmentList" v-permission="TOOLING_BUDGET_INVESTMENT">
		<div>
			...
			<iCard
				...
				class="margin-bottom20 headerIcard"
			...
			>
				...
				<div class="infoIcard">
					<div class="search">
						...
						<span v-show="pageEdit" class="infoIcardValue">{{
							versionName
						}}</span>
					</div>
					<div>
					...
					<div v-show="params.sourceStatus == 1">
						<Popover placement="left" trigger="hover">
							...
							<label slot="reference"
								>{{ $t('MOULDADD.LK_SHENGCHANGONGCHANG') }}:</label
							>
						</Popover>
						<!--            <label :title="$t('LK_CAIGOUGONGCHANG')">{{ $t('LK_CAIGOUGONGCHANG') }}:</label>-->
						<span class="infoIcardValue">{{
							form['search.purchasingFactory']
						}}</span>
					</div>
					<div v-show="params.sourceStatus == 1">
					...
					<div class="search">
						...
						<span v-show="!pageEdit" class="infoIcardValue">{{
							form['search.approvalInvestment']
						}}</span>
						<iInput
							v-show="pageEdit"
							v-model="form['search.approvalInvestment']"
						></iInput>
					</div>
				</div>
				<div id="chart1"></div>
			...
			<iCard v-loading="tableLoading">
				...
				<div class="header margin-bottom20">
					...
					<div>
						<iButton
							v-show="pageEdit"
							@click="pageEdit = false"
							:disabled="
								versionList[0] && form['search.version'] != versionList[0].id
							"
							>{{ $t('LK_TUICHUBIANJI1') }}
						</iButton>
						<iButton v-show="pageEdit" @click="saveRow">{{
							$t('LK_BAOCUN')
						}}</iButton>
						<iButton v-show="pageEdit" @click="saveAsRow">{{
							$t('LK_BAOCUNWEIXINBANBEN')
						}}</iButton>
						<iButton
							v-show="!pageEdit"
							@click="pageEdit = true"
						...
						<iButton v-show="pageEdit" @click="addRow">{{
							$t('MOULDADD.LK_TIANJIAHANG')
						}}</iButton>
						<iButton v-show="pageEdit" @click="deleteIRow">{{
							$t('MOULDADD.LK_SHANCHUHANG')
						}}</iButton>
				...
				<div>
					...
					>
						<template #refCartypeName="scope">
							<div class="linkStyle" v-if="scope.row.refCartypeProId">
								<span @click="clickRefCartypeName(scope.row)">{{
									scope.row.refCartypeName
								}}</span>
							</div>
							<div v-else>
								<span
									@click="clickRefCartypeName(scope.row)"
									style="color: #1663f6; cursor: pointer"
									>-</span
								>
							</div>
						</template>
						<template #refMoldAmount="scope">
							<div v-if="scope.row.refCartypeProId">
								<span>{{
									getTousandNum(Number(scope.row.refMoldAmount).toFixed(2))
								}}</span>
							</div>
							<div v-else>-</div>
						</template>
						<template #budgetAmount="scope">
							<iInput
								...
								@input="changeBudgetAmount(scope.row.budgetAmount)"
								maxlength="20"
							></iInput>
							<div v-if="!pageEdit">
								{{ getTousandNum(Number(scope.row.budgetAmount).toFixed(2)) }}
							</div>
						</template>
						...
						<template #linie="scope">
							...
							<Popover
								...
								trigger="hover"
							>
								<div slot="reference" class="ellipsisDiv">
									{{ scope.row.linieName }}
								</div>
							</Popover>
						</template>
						<template #sourcingType="scope">
							<div v-if="!pageEdit">
								{{ scope.row.sourcingType == 1 ? 'Common' : 'JV' }}
							</div>
							<iSelect
								v-show="pageEdit"
								:placeholder="$t('LK_QINGXUANZE')"
						...
						<template #applyAmount="scope">
							<div class="linkStyle">
								<span @click="clickMoney(scope.row)">{{
									getTousandNum(Number(scope.row.applyAmount).toFixed(2))
								}}</span>
							</div>
						</template>
						<template #nomiAmount="scope">
							<div class="linkStyle">
								<span @click="clickNomiAmountDetail(scope.row)">{{
									getTousandNum(Number(scope.row.nomiAmount).toFixed(2))
								}}</span>
							</div>
						</template>
						<template #baAmount="scope">
							<div class="linkStyle">
								<span @click="clickBaAmountDetail(scope.row)">{{
									getTousandNum(Number(scope.row.baAmount).toFixed(2))
								}}</span>
							</div>
						</template>
						<template #bmAmount="scope">
							<div class="linkStyle">
								<span @click="clickBmAmountDetail(scope.row)">{{
									getTousandNum(Number(scope.row.bmAmount).toFixed(2))
								}}</span>
							</div>
						</template>
					</iTableList>
					<div class="buttomInput">
						...
						<div>
							<h4>AEKO:</h4>
							<iInput
								v-model="form['search.aekoPercent']"
								@input="changePerent"
								:disabled="!pageEdit"
							></iInput>
							% of Sub-Total
						...
						<div>
							...
							<iInput
								v-model="form['search.contingencyPercent']"
								@input="changePerent"
								:disabled="isLocked || !pageEdit"
							></iInput>
							% of Sub-Total
						</div>
						<div>
							...
							<iInput
								v-model="form['search.contingencyAmount']"
								disabled
							></iInput>
						</div>
						<div>
							...
							<iInput
								v-model="form['search.totalBudget']"
								:disabled="isLocked || !pageEdit"
								@input="changeTotalBudget"
							>
								<div slot="suffix" @click="isLocked = !isLocked">
									<icon
										symbol
										name="iconzongyusuansuoding"
										class="icon"
										v-if="isLocked"
									/>
									<icon
										symbol
										name="iconzongyusuanweisuoding"
										class="icon"
										v-if="!isLocked"
									/>
								</div>
		...
		<addRow
			v-model="addRowShow"
			:carTypeProId="
				$store.state.mouldManagement.budgetManagement.carTypeProject
			"
...
export default {
	...
	methods: {
		...
		changePerent() {
			if (this.isLocked) {
				this.form['search.AEKOMoney'] = (
					Number(this.form['search.aekoPercent']) *
					Number(this.form['search.SUBTOTA']) *
					0.01
				).toFixed(2)
				this.form['search.contingencyAmount'] = (
					Number(this.form['search.totalBudget']) -
					Number(this.form['search.SUBTOTA']) -
					Number(this.form['search.AEKOMoney'])
				).toFixed(2)
				this.form['search.contingencyPercent'] = (
					(Number(this.form['search.contingencyAmount']) /
						Number(this.form['search.SUBTOTA'])) *
					100
				).toFixed(2)
			} else {
				...
				this.form['search.contingencyAmount'] = (
					Number(this.form['search.contingencyPercent']) *
					Number(this.form['search.SUBTOTA']) *
					0.01
				).toFixed(2)
				this.form['search.totalBudget'] = (
					Number(this.form['search.AEKOMoney']) +
					Number(this.form['search.contingencyAmount']) +
					Number(this.form['search.SUBTOTA'])
				).toFixed(2)
		...
		conversionSave(val) {
			...
			this.tableListData = cloneDeep(this.tableListDataClone).map((item) => {
				item.budgetAmount = (Number(item.budgetAmount) * conversionVal).toFixed(
					2
				)
				return item
			})
			this.form['search.SUBTOTA'] = (
				Number(this.clone['search.SUBTOTA']) * conversionVal
			).toFixed(2)
			this.changePerent()
		},
		changeVersion(val) {
			if (val) {
				this.versionName = this.versionList.find(
					(item) => item.id == val
				).version
				this.createDate = this.versionList.find(
					(item) => item.id == val
				).createDate
		...
		async getModelProtitesPullDown() {
			...
			await Promise.all([
				...
				.then((res) => {
					const result0 =
						this.$i18n.locale === 'zh' ? res[0].desZh : res[0].desEn
					const result1 =
						this.$i18n.locale === 'zh' ? res[1].desZh : res[1].desEn
					const result2 =
						this.$i18n.locale === 'zh' ? res[2].desZh : res[2].desEn
					const result3 =
						this.$i18n.locale === 'zh' ? res[3].desZh : res[3].desEn
		...
		findInvestmentList() {
			...
			let detpItem = this.DeptPullDown2.find(
				(item) => Number(item.deptId) === Number(this.form['search.DeptSelect'])
			)
			findInvestmentList({
				...
				.then((res) => {
					...
					if (Number(res.code) === 0) {
						let subTotal = 0
						res.data.investmentListEntities = res.data.investmentListEntities
							? res.data.investmentListEntities
							: []
						this.tableListData = res.data.investmentListEntities.map(
							(item, index) => {
								item.index = index
								item.budgetAmount = Number(item.budgetAmount).toFixed(2)
								let linieName = ''
								item.linieArr = item.linie
									? item.linie.split(',').map((key) => Number(key))
									: []
								if (this.$i18n.locale == 'zh') {
									item.linieArr.map((a) => {
										linieName += this.liniePullDown.find((b) => b.linieID == a)
											? this.liniePullDown.find((b) => b.linieID == a)
													.linieName + '/'
											: ''
									})
								} else {
						...
						this.form['search.aekoPercent'] = Number(
							res.data.aekoPercent
						).toFixed(2)
						this.form['search.AEKOMoney'] = (
							Number(this.form['search.aekoPercent']) *
							Number(this.form['search.SUBTOTA']) *
						...
						this.form['search.contingencyPercent'] = Number(
							res.data.contingencyPercent
						).toFixed(2)
						this.form['search.contingencyAmount'] = (
							Number(this.form['search.contingencyPercent']) *
							Number(this.form['search.SUBTOTA']) *
			...
			getInvestmentData({
				...
				.then((res) => {
					...
					if (Number(res.code) === 0) {
						...
						this.form['search.purchasingFactory'] = res.data.purchasingFactory
							? res.data.purchasingFactory.join('')
							: ''
						this.form['search.approvalInvestment'] = Number(
							res.data.approvalInvestment
						)
							? Number(res.data.approvalInvestment).toFixed(2)
							: 0
						...
						this.$nextTick(() => {
								...
								tooltip: {
									formatter: function (params) {
										...
										if (params.seriesIndex == 0) {
											return (
												Number(
													(_this.form['search.contingencyAmount'] /
														_this.form['search.totalBudget']) *
														100
												).toFixed(2) + '%'
											)
								...
								series: [
									{
										...
										data: [
											...
											0,
											Number(notAekoPriceDetail.nomiAmount) +
												Number(notAekoPriceDetail.nomiJAmount),
							...
							option1.series[option1.series.length - 1].label.formatter =
								this.form['search.totalBudget']
				...
				.then((res) => {
					...
					if (Number(res.code) === 0) {
						...
						this.versionName = this.versionList[0]
							...
							: ''
						this.createDate = this.versionList[0]
							? this.versionList[0].createDate
							: ''
						this.findInvestmentList()
					} else {
						iMessage.error(result)
		...
		handleAddCarTypeProject() {
			this.tableLoading = true
			saveCustomCart({ cartypeProjectName: this.addCarTypeProject })
				.then((res) => {
					const result = this.$i18n.locale === 'zh' ? res.desZh : res.desEn
					if (Number(res.code) === 0) {
		...
		deleteIRow() {
			...
			this.tableLoading = true
			investmentDelete(this.selectTableData, {
				investmentVersionId: this.form['search.version'],
			})
				.then((res) => {
					const result = this.$i18n.locale === 'zh' ? res.desZh : res.desEn
		...
		saveAsRow() {
			this.saveParams.investmentListEntities = this.tableListData.map(
				(item) => {
					item.linie = item.linieArr.join(',')
					return item
				}
			...
			this.saveParams.contingencyPercent =
				this.form['search.contingencyPercent']
