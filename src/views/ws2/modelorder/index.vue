<template>
  <div>
    <ModelOrderSeachComponents class="margin-bottom20 margin-top20"
    ...
    <i-card>
      <div class='margin-bottom10 clearFloat'>
        <show-me-components :is-only-myself='orderQueryForm.isOnlyMyself'
                            @showOnlyMyselfData='showOnlyMyselfData'/>
        <!--右侧按钮区-->
        <div class='floatright'>
          <!--新建采购订单-->
          <i-button @click='createOrder'>{{ $t('MODEL-ORDER.LK_XINJIAN') }}</i-button>
          <!--删除采购订单-->
          <i-button @click='deleteOrder'>{{ $t('MODEL-ORDER.LK_SHANCHU') }}</i-button>
          <!--转派-->
          <i-button @click='transferOrder'>{{ $t('MODEL-ORDER.LK_ZHUANPAI') }}</i-button>
          <!--创建入账单-->
          <i-button @click='createIncomingBill'>{{ $t('MODEL-ORDER.LK_CHUANGJIANRUZHANGDAN') }}</i-button>
          <!--报销申请-->
          <i-button @click='reimbursementApplication'>{{ $t('MODEL-ORDER.LK_BAOXIAOSHENQING') }}</i-button>
          <!--导出-->
          <i-button @click='exportOrderData'>{{ $t('MODEL-ORDER.LK_DAOCHU') }}</i-button>
          <!--发送SAP-->
          <i-button @click='sendOrderSAP'>{{ $t('MODEL-ORDER.LK_FASONGSAP') }}</i-button>
          <!--全部展开或收起-->
          <i-button @click='toggleRowExpansion'>
            {{ isExpandAll ? $t('MODEL-ORDER.LK_QUANBUSHOUQI') : $t('MODEL-ORDER.LK_QUANBUZHANKAI') }}
          </i-button>
        </div>
      ...
      <order-home-table-components ref="orderHomeTableComponentsRef" :columns="columns" :order-data="orderData"
                                   ...
                                   @open-page='openPage'/>
      <!--分页区-->
      <i-pagination v-update @size-change='handleSizeChange($event, loadOrder)'
                    @current-change='handleCurrentChange($event, loadOrder)' background
...
export default {
  ...
  methods: {
    ...
    deleteOrder() {
      ...
      let state = this.selectedOrderData.some((i) => i.state == 'formal' || i.state == 'history')
      if (state) return this.$message.warning('选中采购订单包含非草稿状态不支持删除')
      if (this.currentUserPurchasingGroups != null && this.currentUserPurchasingGroups.length > 0 && this.currentUserPurchasingGroups != undefined) {
        let notContainOrders = []
        for (let i = 0; i < this.selectedOrderData.length; i++) {
          let selOrderItem = this.selectedOrderData[i]
          if (this.currentUserPurchasingGroups.indexOf(selOrderItem.procureGroup.toUpperCase()) == -1) {
        ...
        if (notContainOrders.length > 0) {
          ...
          notContainOrders.forEach((item, index) => {
            str_message.push(`您无该${item.contractCode}订单采购组${item.procureGroup}删除权限无法删除`)
          })
    ...
    transferOrder() {
      ...
      if (this.selectedOrderData.find((i) => i.state).state.toUpperCase() !== 'DRAFT') {
    ...
    sendOrderSAP() {
      ...
      purchaseOrderSubmission(this.selectedOrderData.find((i) => i.id).id).then((res) => {
        if (res.code == 200) {
          this.$message.success(res.desZh)
          this.loadDbOrder()
        } else {
